...
'</gml:pos> \
   ...
   function setTitle(title) {
    ...
    function doGUI() {
        ...
        // GRK - stm : function to load a map of story
        function loadStm(stm, index) {
            if (index > stm.length-1) {
                index = stm.length-1;
            }
            if (index < 0) {
                index = 0;
            }
            config.stm.index = index;
            $("#stm-btn-prev").hide();
            $("#stm-btn-next").hide();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    parseWMC(stm[index].wmc_url);
                }
            });
            console.log(stm.length, config.stm.index, index);
        }
        
        // GRK - stm: init story map module
        function storyMap() {
            if (config.stm && config.stm.length>0) {
                // show stm button
                $("#panelStmBtn").show();
            }
            $("#panelStm").on("popupbeforeposition", function(event, ui) {
                // Load first map when panelStm open
                loadStm(config.stm, 0);
            });
        }
        storyMap();
        
        // GRK - stm next button
        $("#stm-btn-next").on("click", function(){
            loadStm(config.stm, config.stm.index+1);
        });
        // GRK - stm previous button
        $("#stm-btn-prev").on("click", function(){
            loadStm(config.stm, config.stm.index-1);
        });
